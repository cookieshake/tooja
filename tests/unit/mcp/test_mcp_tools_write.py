...
@pytest.mark.asyncio
async def test_create_invalid_qty_rejected():
    fb = FakeBroker()

    async def must_not_be_called(req):
        raise AssertionError("broker must not be called for invalid qty")
    ...
    out = await orders_tools.create(
        ...
        symbol="005930", side="buy", qty="ten", type="limit", price="70000",
    )
