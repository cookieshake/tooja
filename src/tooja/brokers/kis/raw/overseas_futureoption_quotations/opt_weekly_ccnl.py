...
class OptWeeklyCcnlRequest(KisBaseModel):
    """요청."""
...
class OptWeeklyCcnlResponse_Output1Item(KisBaseModel):
    """nested item."""
...
class OptWeeklyCcnlResponse_Output2Item(KisBaseModel):
    """nested item."""
...
class OptWeeklyCcnlResponse(KisCommonResponse):
    """응답 본문."""

    output1: OptWeeklyCcnlResponse_Output1Item | None = None  # 응답상세
    output2: list[OptWeeklyCcnlResponse_Output2Item] = []  # 응답상세 — array

class OptWeeklyCcnlExecutor(ApiExecutor[OptWeeklyCcnlRequest, OptWeeklyCcnlResponse]):
    """해외옵션 체결추이(주간) [해외선물-036]."""

    # 해외옵션 체결추이(주간) API입니다. 최근 120건까지 데이터 확인이 가능합니다. (START_DATE_TIME, CLOSE_DATE_TIME은 공란 입력) (중요) 해외옵션시세 출력값을 해석하실 때 focode.mst(해외지수옵션 종목마스터파일), fostkcode.mst(해외주식옵션 종목마스터파일)에 있는 sCalcDesz(계산 소수점) 값을 활용하셔야 정확한 값을 받아오실 수 있습니다. - focode.mst(해외지수옵

    PATH = "/uapi/overseas-futureoption/v1/quotations/opt-weekly-ccnl"
    METHOD = "GET"
    RESPONSE_TYPE = OptWeeklyCcnlResponse
    TR_ID = "HHDFO55020000"
