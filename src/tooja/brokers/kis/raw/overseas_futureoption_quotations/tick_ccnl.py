...
class TickCcnlRequest(KisBaseModel):
    """요청."""
...
class TickCcnlResponse_Output1Item(KisBaseModel):
    """nested item."""
...
class TickCcnlResponse_Output2Item(KisBaseModel):
    """nested item."""
...
class TickCcnlResponse(KisCommonResponse):
    """응답 본문."""

    output1: TickCcnlResponse_Output1Item | None = None  # 응답상세
    output2: list[TickCcnlResponse_Output2Item] = []  # 응답상세 — array

class TickCcnlExecutor(ApiExecutor[TickCcnlRequest, TickCcnlResponse]):
    """해외선물 체결추이(틱)[해외선물-019]."""

    # 해외선물옵션 체결추이(틱) API입니다. 한국투자 HTS(eFriend Plus) &gt; [5502] 해외선물옵션 체결추이 화면에서 "Tick" 선택 시 기능을 API로 개발한 사항으로, 해당 화면을 참고하시면 기능을 이해하기 쉽습니다. (중요) 해외선물시세 출력값을 해석하실 때 ffcode.mst(해외선물종목마스터 파일)에 있는 sCalcDesz(계산 소수점) 값을 활용하셔야 정확한 값을 받아오실 수 있습니다. - ffco

    PATH = "/uapi/overseas-futureoption/v1/quotations/tick-ccnl"
    METHOD = "GET"
    RESPONSE_TYPE = TickCcnlResponse
    TR_ID = "HHDFC55020200"
