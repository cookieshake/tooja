...
class OptDailyCcnlRequest(KisBaseModel):
    """요청."""
    ...
    QRY_TP: str  # 조회구분 — Q
    QRY_CNT: str  # 요청개수 — 예) 100 (최대 119) ※ QRY_CNT 입력값의 +1 개 데이터가 조회됩니다.
    QRY_GAP: str  # 묶음개수 — "" 공란 입력
    INDEX_KEY: str  # 이전조회KEY — "" 공란 입력 ※ 다음조회 불가

class OptDailyCcnlResponse_Output1Item(KisBaseModel):
    """nested item."""

    ret_cnt: str | None = None  # 자료개수 — ※ "input > QRY_CNT" +1 개 만큼 조회됨
    last_n_cnt: str | None = None  # N틱최종개수
    index_key: str | None = None  # 이전조회KEY

class OptDailyCcnlResponse_Output2Item(KisBaseModel):
    """nested item."""
...
class OptDailyCcnlResponse(KisCommonResponse):
    """응답 본문."""

    output1: OptDailyCcnlResponse_Output1Item | None = None  # 응답상세
    output2: list[OptDailyCcnlResponse_Output2Item] = []  # 응답상세 — array

class OptDailyCcnlExecutor(ApiExecutor[OptDailyCcnlRequest, OptDailyCcnlResponse]):
    """해외옵션 체결추이(일간) [해외선물-037]."""

    # 해외옵션 체결추이(일간) API입니다. 최근 120건까지 데이터 확인이 가능합니다. ("QRY_CNT: 119 입력", START_DATE_TIME, CLOSE_DATE_TIME은 공란) ※ 호출 시 유의사항 : START_DATE_TIME, CLOSE_DATE_TIME은 공란 입력, QRY_CNT는 확인 데이터 개수의 -1 개 입력 ex) "START_DATE_TIME":"","CLOSE_DATE_TIME":"","QRY_C

    PATH = "/uapi/overseas-futureoption/v1/quotations/opt-daily-ccnl"
    METHOD = "GET"
    RESPONSE_TYPE = OptDailyCcnlResponse
    TR_ID = "HHDFO55020100"
