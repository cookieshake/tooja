...
class NgtMarginDetailResponse_Output3Item(KisBaseModel):
    ...
    bfdy_sbst_sll_sbst_amt: str | None = None  # 전일대용매도대용금액 — 신 TR 사용 필드
    thdt_sbst_sll_sbst_amt: str | None = None  # 당일대용매도대용금액 — 신 TR 사용 필드
    bfdy_sbst_sll_ccld_amt: str | None = None  # 전일대용매도체결금액 — 신 TR 사용 필드
    thdt_sbst_sll_ccld_amt: str | None = None  # 당일대용매도체결금액 — 신 TR 사용 필드
    opt_dfpa: str | None = None  # 옵션차금 — 신 TR 사용 필드
    excc_dfpa: str | None = None  # 정산차금 — 신 TR 사용 필드
    fee_amt: str | None = None  # 수수료금액 — 신 TR 사용 필드
    nxdy_dncl_amt: str | None = None  # 익일예수금액 — 신 TR 사용 필드
    prsm_dpast_amt: str | None = None  # 추정예탁자산금액 — 신 TR 사용 필드
    opt_buy_exus_acnt_yn: str | None = None  # 옵션매수전용계좌여부 — 신 TR 사용 필드
    base_dpsa_gdat_grad_cd: str | None = None  # 기본예탁금차등등급코드 — 신 TR 사용 필드
    opt_base_dpsa_gdat_grad_cd: str | None = None  # 옵션기본예탁금차등등급코드 — 신 TR 사용 필드
...
class NgtMarginDetailResponse(KisCommonResponse):
    ...
    output1: list[NgtMarginDetailResponse_Output1Item] = []  # 응답상세 — Array 신 TR 사용 필드
    output2: list[NgtMarginDetailResponse_Output2Item] = []  # 응답상세 — Array 신 TR 사용 필드
    output3: NgtMarginDetailResponse_Output3Item | None = None  # 응답상세 — Single 신 TR 사용 필드
