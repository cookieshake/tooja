...
class ProfitAssetIndexRequest(KisBaseModel):
    """요청."""
...
class ProfitAssetIndexResponse_OutputItem(KisBaseModel):
    """nested item."""
...
class ProfitAssetIndexResponse(KisCommonResponse):
    """응답 본문."""

    output: list[ProfitAssetIndexResponse_OutputItem] = []  # 응답상세 — array

class ProfitAssetIndexExecutor(ApiExecutor[ProfitAssetIndexRequest, ProfitAssetIndexResponse]):
    """국내주식 수익자산지표 순위[v1_국내주식-090]."""

    # 국내주식 수익자산지표 순위 API입니다. 한국투자 HTS(eFriend Plus) &gt; [0173] 수익자산지표 순위 화면의 기능을 API로 개발한 사항으로, 해당 화면을 참고하시면 기능을 이해하기 쉽습니다. 최대 30건 확인 가능하며, 다음 조회가 불가합니다. ※ 30건 이상의 목록 조회가 필요한 경우, 대안으로 종목조건검색 API를 이용해서 원하는 종목 100개까지 검색할 수 있는 기능을 제공하고 있습니다. 종목조건검색

    PATH = "/uapi/domestic-stock/v1/ranking/profit-asset-index"
    METHOD = "GET"
    RESPONSE_TYPE = ProfitAssetIndexResponse
    TR_ID = "FHPST01730000"
