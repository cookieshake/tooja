...
class TradeTurnoverRequest(KisBaseModel):
    """요청."""
...
class TradeTurnoverResponse_Output1Item(KisBaseModel):
    """nested item."""
...
class TradeTurnoverResponse_Output2Item(KisBaseModel):
    """nested item."""
    ...
    n_tvol: str | None = None  # 평균거래량
    shar: str | None = None  # 상장주식수
    tover: str | None = None  # 회전율
    rank: str | None = None  # 순위
    ename: str | None = None  # 영문종목명
    e_ordyn: str | None = None  # 매매가능

class TradeTurnoverResponse(KisCommonResponse):
    """응답 본문."""

    output1: TradeTurnoverResponse_Output1Item | None = None  # 응답상세
    output2: list[TradeTurnoverResponse_Output2Item] = []  # 응답상세 — array

class TradeTurnoverExecutor(ApiExecutor[TradeTurnoverRequest, TradeTurnoverResponse]):
    """해외주식 거래회전율순위[해외주식-046]."""

    # 해외주식 거래회전율순위 API입니다. 한국투자 HTS(eFriend Plus) &gt; [7634] 거래회전율순위 화면 기능을 API로 개발한 사항으로, 해당 화면을 참고하시면 기능을 이해하기 쉽습니다.

    PATH = "/uapi/overseas-stock/v1/ranking/trade-turnover"
    METHOD = "GET"
    RESPONSE_TYPE = TradeTurnoverResponse
    TR_ID = "HHDFS76340000"
