...
class TradeGrowthRequest(KisBaseModel):
    """요청."""
...
class TradeGrowthResponse_Output1Item(KisBaseModel):
    """nested item."""
...
class TradeGrowthResponse_Output2Item(KisBaseModel):
    """nested item."""
...
class TradeGrowthResponse(KisCommonResponse):
    """응답 본문."""

    output1: TradeGrowthResponse_Output1Item | None = None  # 응답상세
    output2: list[TradeGrowthResponse_Output2Item] = []  # 응답상세 — array

class TradeGrowthExecutor(ApiExecutor[TradeGrowthRequest, TradeGrowthResponse]):
    """해외주식 거래증가율순위[해외주식-045]."""

    # 해외주식 거래증가율순위 API입니다. 한국투자 HTS(eFriend Plus) &gt; [7633] 거래증가율순위 화면 기능을 API로 개발한 사항으로, 해당 화면을 참고하시면 기능을 이해하기 쉽습니다.

    PATH = "/uapi/overseas-stock/v1/ranking/trade-growth"
    METHOD = "GET"
    RESPONSE_TYPE = TradeGrowthResponse
    TR_ID = "HHDFS76330000"
