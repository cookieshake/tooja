...
class VolumePowerRequest(KisBaseModel):
    ...
    NDAY: str  # N일자값 — N분전 : 0(1분전), 1(2분전), 2(3분전), 3(5분전), 4(10분전), 5(15분전), 6(20분전), 7(30분전), 8(60분전), 9(120분전)
    VOL_RANG: str  # 거래량조건 — 0(전체), 1(1백주이상), 2(1천주이상), 3(1만주이상), 4(10만주이상), 5(100만주이상), 6(1000만주이상)

class VolumePowerResponse_Output1Item(KisBaseModel):
    """nested item."""
...
class VolumePowerResponse_Output2Item(KisBaseModel):
    """nested item."""
...
class VolumePowerResponse(KisCommonResponse):
    ...
    output1: VolumePowerResponse_Output1Item | None = None  # 응답상세
    output2: list[VolumePowerResponse_Output2Item] = []  # 응답상세 — array

class VolumePowerExecutor(ApiExecutor[VolumePowerRequest, VolumePowerResponse]):
    """해외주식 매수체결강도상위[해외주식-040]."""

    # 해외주식 매수체결강도상위 API입니다. 한국투자 HTS(eFriend Plus) &gt; [7628] 매수체결강도상위 화면 기능을 API로 개발한 사항으로, 해당 화면을 참고하시면 기능을 이해하기 쉽습니다.
